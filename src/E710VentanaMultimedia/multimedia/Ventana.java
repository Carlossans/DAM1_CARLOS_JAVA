...
public class Ventana extends JFrame {
	...
	private b700proyecto.E710VentanaMultimedia.multimedia.Teclado teclado = new b700proyecto.E710VentanaMultimedia.multimedia.Teclado();



	public Ventana(String tituloVentana, int ancho, int alto, int tamPixel, Color colorFondo) {
		super(tituloVentana);
    ...
    // TODO: este no es el lugar ideal para que esté este método pero por ahora nos vale.
    public BufferedImage cargarImagen(String ruta) {
        try {
            return ImageIO.read(new File(ruta));
        } catch (Exception e) {
            System.err.println("Error cargando la imagen: " + ruta);
            return null;
        }
    }
    ...
    public void dibujarImagen(int x, int y, BufferedImage imagen) {
        if (imagen != null) {
            // Los parámetros indican: imagen, x, y, ancho, alto, observador
            // Al pasarle 'tamPixel' como ancho y alto, Java la escala automáticamente
            // para que encaje perfectamente en tu celda.
            imagenG2D.drawImage(imagen, x * tamPixel, y * tamPixel, tamPixel, tamPixel, null);
        }
    }
